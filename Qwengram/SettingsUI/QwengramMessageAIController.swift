...
private final class QwengramMessageAIArguments {
...
private enum QwengramMessageAIEntry: ItemListNodeEntry {
    ...
    static func == (lhs: QwengramMessageAIEntry, rhs: QwengramMessageAIEntry) -> Bool {
...
public func qwengramMessageAIController(context: AccountContext, text: String) -> ViewController {
    var pushControllerImpl: ((ViewController) -> Void)?
    let arguments = QwengramMessageAIArguments(
        openAssistant: {
            pushControllerImpl?(qwengramQwenAssistantController(context: context, initialText: text))
        },
        openSummarizer: {
            pushControllerImpl?(qwengramSummarizerController(context: context, initialText: text))
        },
        openTranslator: {
            pushControllerImpl?(qwengramTranslatorController(context: context, initialText: text))
        }
    )
    let signal = context.sharedContext.presentationData
    |> map { presentationData -> (ItemListControllerState, (ItemListNodeState, Any)) in
        let listPresentationData = ItemListPresentationData(presentationData)
        let entries: [QwengramMessageAIEntry] = [
        ...
        let controllerState = ItemListControllerState(presentationData: listPresentationData, title: .text("Qwengram AI"), leftNavigationButton: nil, rightNavigationButton: nil, backNavigationButton: ItemListBackButton(title: presentationData.strings.Common_Back))
